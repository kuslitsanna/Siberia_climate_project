...
siberia_shape <- read_sf("Siberia_shape.kml") # simple shapefile, rough approximation created in Google Earth available in project repo
...
unfiltered_stations <- bq_project_query(projectid, sql) %>% bq_table_download() # query returns 1267 weather stations 
...
filtered_stations <- unfiltered_stations %>% st_intersection(siberia_shape$geometry) # 667 stations
...
# reduce number of stations to those located at a minimum distance of 300 km from each other
selected_stations <- filterByProximity(filtered_stations, dist=300, returnIndex=FALSE) # 105 stations selected

# visualise location of selected stations
basemap %>% addCircleMarkers(data=selected_stations, radius=1)

# adjust variable types
selected_stations <- selected_stations %>% mutate(id = as.factor(id), name = as.factor(name))


## Export outputs ####

# create a directory named 'outputs' in working directory 
output_folder <- file.path(getwd(), "outputs")
dir.create(output_folder, showWarnings = FALSE)


# export selected_stations object to kml file
st_write(selected_stations, file.path(output_folder, "selected_stations.kml"))

# print map of selected station locations with labels displaying name, long, lat 
map_stations <- basemap %>%
  addMarkers(data = selected_stations, label = paste(selected_stations$name, selected_stations$id, "Long:", selected_stations$longitude, "Lat:", selected_stations$latitude, sep = " "))
map_stations

# save map as an HTML file to retain interactive features
saveWidget(map_stations,file.path(output_folder, "interactive_map_selected_stations.html"))
